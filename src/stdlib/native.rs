...
pub fn native_base64(args: &[Value], line: usize, ctx: &Ctx) -> Result<Value, RuntimeError> {
    ...
    let written = unsafe {
        rach_base64_encode(bytes.as_ptr(), bytes.len(), buf.as_mut_ptr().cast::<c_char>())
    };
...
pub fn native_sort_ints(args: &[Value], line: usize, ctx: &Ctx) -> Result<Value, RuntimeError> {
    ...
    let rc = unsafe { rach_sort_csv_ints(owned.as_mut_ptr().cast::<c_char>()) };
    if rc != 0 {
        return Err(RuntimeError::new(400, line, "native_sort_ints: failed to parse comma-separated integers"));
    }
    let result = unsafe { CStr::from_ptr(owned.as_ptr().cast::<c_char>()) }
        .to_string_lossy()
        .into_owned();
    if !ctx.capturing {
...
pub fn native_reverse(args: &[Value], line: usize, ctx: &Ctx) -> Result<Value, RuntimeError> {
    ...
    let mut owned = cstr.into_bytes_with_nul();
    unsafe { rach_reverse_bytes(owned.as_mut_ptr().cast::<c_char>()) };
    let result = unsafe { CStr::from_ptr(owned.as_ptr().cast::<c_char>()) }
        .to_string_lossy()
        .into_owned();
    if !ctx.capturing {
...
fn spawn_native(lang: &str, code: &str, line: usize, ctx: &Ctx) -> Result<Value, RuntimeError> {
    ...
    let src_path = tmpdir.join(format!("rach_run_{lang}_{stamp}.{src_ext}"));
    let bin_path = tmpdir.join(format!("rach_run_{lang}_{stamp}"));
