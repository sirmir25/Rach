...
pub fn click_button(args: &[Value], line: usize, ctx: &Ctx) -> Result<Value, RuntimeError> {
    ...
    let xpath = format!(
        "//button[normalize-space()={escaped}] | \
         //input[(@type='submit' or @type='button') and (@value={escaped} or @aria-label={escaped})] | \
         //*[@role='button' and (normalize-space()={escaped} or @aria-label={escaped})] | \
...
pub fn type_text(args: &[Value], line: usize, ctx: &Ctx) -> Result<Value, RuntimeError> {
    let id = args.first().map(Value::as_str).unwrap_or_default();
...
pub fn login(kwargs: &BTreeMap<String, Vec<Value>>, line: usize, ctx: &Ctx) -> Result<Value, RuntimeError> {
    ...
    println!("(login) user={user:?} password=***");
    println!("completed");
    Ok(Value::Nil)
}
...
pub fn download_file(args: &[Value], line: usize) -> Result<Value, RuntimeError> {
    ...
    let path = args.get(1).map(Value::as_str).unwrap_or_default();
    ...
    match r {
        ...
        _ => { eprintln!("error 502 string {line}  // download failed"); Ok(Value::Nil) }
    }
}
...
pub fn upload_file(args: &[Value], line: usize, ctx: &Ctx) -> Result<Value, RuntimeError> {
    ...
    let id = args.get(1).map(Value::as_str).unwrap_or_default();
